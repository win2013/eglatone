/* global eglatoneOptions */
 /*
 * Custom scripts
 * Description: Custom scripts for eglatone
 */
...
( function( $ ) {
	...
	if ( typeof $.fn.owlCarousel === "function" ) {
		// Featured Slider
		var sliderOptions = {
			rtl:eglatoneOptions.rtl ? true : false,
			autoHeight:true,
			margin: 0,
			items: 1,
			nav: false,
	...
	function initMainNavigation( container ) {
		...
		var dropdownToggle = $( '<button />', { 'class': 'dropdown-toggle', 'aria-expanded': false })
			.append( $( '<span />', { 'class': 'screen-reader-text', text: eglatoneOptions.screenReaderText.expand }) );
		...
		container.find( '.dropdown-toggle' ).on( 'click', function( e ) {
			...
			// jscs:enable
			screenReaderSpan.text( screenReaderSpan.text() === eglatoneOptions.screenReaderText.expand ? eglatoneOptions.screenReaderText.collapse : eglatoneOptions.screenReaderText.expand );
		} );
	}
	...
	( function() {
		...
		menuToggle.on( 'click.eglatone', function() {
			// jscs:disable
	...
	( function() {
		...
		function toggleFocusClassTouchScreen() {
			if ( window.innerWidth >= 910 ) {
				$( document.body ).on( 'touchstart.eglatone', function( e ) {
					if ( ! $( e.target ).closest( '.main-navigation li' ).length ) {
						$( '.main-navigation li' ).removeClass( 'focus' );
					}
				} );
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.eglatone', function( e ) {
				...
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).unbind( 'touchstart.eglatone' );
			}
		}

		if ( 'ontouchstart' in window ) {
			$( window ).on( 'resize.eglatone', toggleFocusClassTouchScreen );
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find( 'a' ).on( 'focus.eglatone blur.eglatone', function() {
